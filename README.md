...
* 🔐 **Secure Admin Gate (Integer Token Encryption):** Sistem pendaftaran (*Sign Up*) dan masuk (*Sign In*) Admin yang dilengkapi CAPTCHA matematika. Guna menghindari kerentanan algoritma enkripsi biner terhadap penanda akhir file (*End-Of-File* `\x1A`) pada OS Windows atau karakter *Enter* (`\n`, `\r`) yang dapat memotong baris teks database secara sepihak, sistem ini mengubah hasil operasi biner XOR `'C'` langsung menjadi deretan string angka indeks tekstual terpisah yang $100\%$ aman di dalam file `data_atmint.txt`.
...
* 🎮 **Maneki Neko Engine (60FPS Smooth Arcade):** Game interaktif penangkap koin emas yang dirender langsung menggunakan teknik pemformatan string tunggal tunggal (*Double Buffering Text*) serta penyembunyian kursor (`\033[?25l`), menghasilkan pergerakan animasi koin yang mulus tanpa kedipan terminal.
...
├── 📄 data_atmint.txt          # [Auto-Generated] Database Akun Admin (Secure Integer Token)
└── 📄 stok_barang.txt          # [Auto-Generated] Database Stok Barang (Format CSV)
...
| **Ni Ketut Intan Ayu W** | `admin 1.cpp` | Logika gerbang masuk admin, validasi struktur format NIM, dan pembuatan fungsi input penambahan menu baru. |
...
| **Baiq Nur Saqinah K** | `customerTransaction.cpp` | Alur antarmuka transaksi pembeli, kalkulasi total belanja ditambah pajak restoran 11% + biaya layanan, dan format visual nota digital. |
| **Rizka Dzuliyatun N** | `undian.cpp` | Manajemen struktur data hadiah gacha global, penentuan persentase acak probabilitas hadiah, dan menu penukaran *loyalty* poin. |
| **I Dewa Made K W** | `poin.cpp` | Modul perhitungan penambahan poin *loyalty* (tiap kelipatan Rp 10.000) serta pencatatan akumulasi pendapatan omzet kafe harian. |
...
2. Ketik perintah di bawah ini untuk berpindah tempat ke lokasi folder yang Anda inginkan (misal: folder *Documents*):
```bash
cd Documents
...
Jika ada anggota kelompok lain (misalnya Arizona atau Intan) yang melakukan pembaruan kode atau perbaikan bug di GitHub, Anda tidak perlu mendownload ulang dari awal. Cukup sinkronkan folder Anda dengan langkah berikut:
...
2. Gradien warna diagonal cyberpunk *True Color 24-bit* dapat menyala dengan tajam, rapi, dan presisi di layar monitor dosen penguji.

```
_

README ini sudah siap sepenuhnya untuk ditaruh di repositori proyek kelompok kalian! Semua instruksi telah diselaraskan dengan modifikasi mutakhir yang kita buat pada fungsi enkripsi token dan manipulasi kursor. Jika ada hal lain yang ingin kamu tambahkan untuk persiapan demonstrasi, beri tahu saya, Arizona!
